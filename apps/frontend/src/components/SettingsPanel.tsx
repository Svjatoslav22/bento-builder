...
export default function SettingsPanel({ selectedWidget, activeWidgetType: activeWidgetTypeProp, selectedSize, onSizeChange, onRemove, avatarUrl, onAvatarChange, city, timezone, onCityChange, onTimezoneChange, profileFields, onProfileChange, activeWidget, updateWidget }: SettingsPanelProps) {
  ...
  return (
    <aside ref={panelRef} className="w-full md:w-[320px] h-full bg-surface border-l border-border flex flex-col flex-shrink-0 z-20">
      ...
      <div className="p-5 flex-1 overflow-y-auto space-y-6">
        ...
        {activeWidgetType === "portfolio" && <label className="block text-xs font-medium text-text-secondary">Portfolio URL<input value={String(widgetContent.url || "")} onChange={(event) => updateContent("url", event.target.value)} className={`${inputClass} mt-1.5`} type="url" placeholder="https://yourportfolio.com" /></label>}
        {activeWidgetType === "resume" && <label className="block text-xs font-medium text-text-secondary">Resume URL (Google Drive link etc.)<input value={String(widgetContent.resumeUrl || "")} onChange={(event) => updateContent("resumeUrl", event.target.value)} className={`${inputClass} mt-1.5`} type="url" placeholder="https://drive.google.com/..." /></label>}
        {activeWidgetType === "ai-chat" && <label className="block text-xs font-medium text-text-secondary">Контекст для AI (Розкажи про себе для HR)<textarea value={String(widgetContent.context || "")} onChange={(event) => updateContent("context", event.target.value)} rows={6} className={`${inputClass} mt-1.5 resize-none`} placeholder="Розкажи про свій досвід, навички та проєкти..." /></label>}
