  ...
  return (
    <aside ref={panelRef} className="w-full md:w-[320px] h-full bg-surface border-l border-border flex flex-col flex-shrink-0 z-20">
      ...
      <div className="p-5 flex-1 overflow-y-auto space-y-6">
        ...
        {safeWidgetType === "tech-stack" && (
          <label className="block text-xs font-medium text-text-secondary">
            Technologies
            <input
              value={String(widgetContent.technologies ?? "")}
              onChange={(event) => updateContent("technologies", event.target.value)}
              className={`${inputClass} mt-1.5`}
              placeholder="React, Next.js, Docker, Prisma"
            />
            <span className="mt-1.5 block text-[11px] font-normal text-text-secondary">
              Перелічіть технології через кому. Зберігається в config віджета.
            </span>
          </label>
        )}
        {safeWidgetType === "github-stats" && (
          <label className="block text-xs font-medium text-text-secondary">
            GitHub username
            <input
              value={String(widgetContent.username || "")}
              onChange={(event) => updateContent("username", event.target.value)}
              className={`${inputClass} mt-1.5`}
              placeholder="octocat"
            />
            <span className="mt-1.5 block text-[11px] font-normal text-text-secondary">
              Якщо порожньо — використовується GitHub URL з Profile Info.
            </span>
          </label>
        )}
