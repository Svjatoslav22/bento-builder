...
export default function SettingsPanel({ selectedWidget, activeWidgetType: activeWidgetTypeProp, selectedSize, onSizeChange, onRemove, avatarUrl, onAvatarChange, city, timezone, onCityChange, onTimezoneChange, profileFields, onProfileChange }: SettingsPanelProps) {
  ...
  return (
    <aside ref={panelRef} className="w-full md:w-[320px] h-full bg-surface border-l border-border flex flex-col flex-shrink-0 z-20">
      <div className="p-5 border-b border-border"><h2 className="text-sm font-semibold text-text-primary">Widget Settings</h2><p className="text-xs text-text-secondary mt-1">{activeWidgetType} Component</p></div>
      <div className="p-5 flex-1 overflow-y-auto space-y-6">
        ...
        {activeWidgetType === "resume" && <div className="space-y-2"><label className="text-xs font-medium text-text-secondary">Resume PDF</label><input ref={inputRef} type="file" accept="application/pdf" className="hidden" onChange={(event) => setFileName(event.target.files?.[0]?.name || "")} /><button type="button" onClick={() => inputRef.current?.click()} className={`${inputClass} text-left`}>↑ Choose PDF file</button>{fileName && <p className="truncate text-xs text-text-secondary">{fileName}</p>}</div>}
